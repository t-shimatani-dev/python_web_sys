...
def index():
    ...
    try:
        ...
        # 例外時にも接続を閉じるため try/finally で処理
        try:
            cursor = conn.cursor()

            # 一覧表示に必要な列だけを取得し、employee_id 順でソート
            cursor.execute("""
                SELECT employee_id, name, name_kana, department,
                position, hire_date, email
                FROM employees
        ...
        try:
            ...
            cursor.execute("""
                INSERT INTO employees (
                    ...
                    hire_date, salary, email, phone, postal_code,
                    address, notes
                ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """, (
                employee_id, name, name_kana, department, position,
...
def search():
    ...
    try:
        ...
        sql = f"""
            SELECT employee_id, name, name_kana, department, position,
                   hire_date, email
            FROM employees
            WHERE {where_sql}
            ORDER BY employee_id
