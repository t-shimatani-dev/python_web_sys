...
- [詳細設計書](#詳細設計書)
  ...
  - [ドキュメント情報](#ドキュメント情報)
  - [変更履歴](#変更履歴)
  - [D-01: データベース初期化機能（移行前参考）](#d-01-データベース初期化機能移行前参考)
    - [概要](#概要)
    - [クラス名](#クラス名)
    - [ファイルパス](#ファイルパス)
    - [処理フロー](#処理フロー)
...
> 補足: 本節は sqlite3 直接操作を使っていた移行前実装の参考です。
> 現行実装は `D-01-B: SQLAlchemy ORM による実装（移行ガイド）` を正としてください。
...
> 移行前例では `conn.row_factory = sqlite3.Row` を設定していました。
> 現行実装では `conn.dbapi_connection.row_factory = sqlite3.Row` を設定しています。
>
...
本節は移行前（sqlite3 版）の説明です。現行実装では SQLAlchemy モデルを生成して `session.add()`/`session.commit()` で保存し、重複時は `ValueError` を送出します。
...
| E005 | 社員ID重複 | INFO | CSVインポートでは重複をスキップ（冪等） |
| E006 | メールアドレス重複 | INFO | CSVインポートでは重複をスキップ（冪等） |
...
`sqlite3` の直接操作を SQLAlchemy 中心に置き換えつつ、既存ルート互換のため `get_connection()` で sqlite3 ライクな接続APIも提供します。
    ...
    <form method="POST">
        ...
        <div class="form-group">
            ...
            <input type="text" name="name_kana" class="form-control" 
                   value="{{ form_data.name_kana if form_data else '' }}" required>
        </div>
                
        <div class="form-group">
            <label>部署 *</label>
            <input type="text" name="department" class="form-control" 
                   value="{{ form_data.department if form_data else '' }}" required>
        </div>
        
        <div class="form-group">
            <label>役職 *</label>
            <input type="text" name="position" class="form-control" 
                   value="{{ form_data.position if form_data else '' }}" required>
        </div>
        
        <div class="form-group">
            <label>入社日 *</label>
            <input type="date" name="hire_date" class="form-control" 
                   value="{{ form_data.hire_date if form_data else '' }}" required>
        </div>
        
        <div class="form-group">
            <label>給与 *</label>
            <input type="number" name="salary" class="form-control" 
                   value="{{ form_data.salary if form_data else '' }}" required>
        </div>
...
<div class="container">
        ...
        <div class="row g-3">
            <div class="col-md-4">
                ...
                <input type="text" class="form-control" id="name" name="name"
                       value="{{ query_name }}" placeholder="氏名で検索">
            </div>
            <div class="col-md-4">
                <label for="department" class="form-label">部署</label>
                <input type="text" class="form-control" id="department" name="department"
                       value="{{ query_department }}" placeholder="部署で検索">
            </div>
            <div class="col-md-4">
                <label for="position" class="form-label">役職</label>
                <input type="text" class="form-control" id="position" name="position"
                       value="{{ query_position }}" placeholder="役職で検索">
            </div>
        </div>
        <div class="mt-3">
            <button type="submit" class="btn btn-primary">検索</button>
            <a href="/search" class="btn btn-secondary">クリア</a>
        </div>
    </form>
    ...
    {% else %}
        <p class="text-muted">該当する社員が見つかりませんでした。</p>
    {% endif %}
